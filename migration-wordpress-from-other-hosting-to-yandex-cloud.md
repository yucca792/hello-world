...
description: "Пошаговая инструкция, с помощью которой вы научитесь вы научитесь разворачивать LAMP в инфраструктуре Yandex.Cloud, переносить данные и настройки WordPress, переводить веб-сайт на протокол **HTTPS** с использованием SSL сертификата от Let's Encrypt."
---
...
С помощью этой инструкции вы научитесь разворачивать LAMP в инфраструктуре {{ yandex-cloud }}, подготавливать виртуальную машину (ВМ) для развертывания единственного сайта, переносить данные и настройки WordPress, переводить веб-сайт на протокол **HTTPS** с использованием SSL сертификата от Let's Encrypt."

Чтобы настроить LAMP-стек:
1. [Подготовьте облако к работе](#before-you-begin).
1. [Создайте виртуальную машину с предустановленным веб-сервером](#create-vm-lamp).
1. [Проверьте работу сайта по внешнему IP-адресу](#test-site-ext-address).
1. [Настройте виртуальный хост для вашего домена](#configure-virthost-http).
1. [Настройте DNS](#configure-dns).
1. [Настройте сетевой экран ВМ](#configure-ufw).
1. [Проверьте работу сайта по доменному имени](#test-site-cname).
1. [Создайте **SSL** cертификат Let’s Encrypt для **Apache**](#create-ssl-le-http-apache).
1. [Настройте доступ к веб-сайту по протоколу **HTTPS**](#configure-access-https-apache).
1. [Настройте автоматическое обновление сертификата](#configure-le-renew).
1. [Настройте расширенные настройки безопасности **Apache**](#configure-advanced-security-apache).
1. [Настройте переадресацию (редирект) запросов по **HTTP**](#configure-redirect-https-apache).
1. [Установите дополнительные пакеты php для **Wordpress**](#install-add-php-for-wordpress}).
1. [Подготовьте резервную копию базы данных **WordPress**](#create-dump-mysql-migration-wordpress).
1. [Подготовьте архив всех файлов **WordPress**](#create-arсhive-all-files-migration-wordpress).
1. [Загрузите резервную копию базы данных **WordPress** с компьютера в домашний каталог](#upload-dump-from-pc-to-home-catalog).
1. [Настройте права доступа к каталогам и файлам сайта-приемника](#configure-access-rights-apache).
1. [Восстановите базу данных **Wordpress** из резервной копии](#recovery-database-wordpress-from-dump).
1. [Настройте **WordPress** для работы в защищенном режиме](#configure-wordpress-https).
1. [Настройте веб-сайт **WordPress** для правильной индексации по **HTTPS**](#configure-wordpress-sitemap-robot-https).
1. [Выполните переезд сайта с **HTTP** на **HTTPS** в **Google Search Console**](#migration-hhtp-https-google).
1. [Выполните переезд сайта с **HTTP** на **HTTPS** в **Яндекс.Вебмастер**](#migration-hhtp-https-yandex).
...
`Certbot` должен иметь возможность найти виртуальный хост для вашего домена в конфигурации **Apache**, чтобы создать сертификат **SSL**. Поиск происходит по директиве `ServerName`, где указано доменное имя. 
...
Для защиты на веб-сервере нужно включить сетевой экран **UFW** и настроить его для поддержки входящих соединений по протоколам **SSH** (22), **HTTP**(80) и **HTTPS**(443).
...
sudo ufw show added
```
Вывод правил на вновь установленной ВМ должен быть пустым.

{% note tip %}

Разрешенные правила UFM выводятся даже при незапущенном (неактивном) сервисе.
...
Вместо записи `CNAME` может быть настроена вторая А-запись, где поддомен `www`, а в качестве адреса используйте публичный IP-адрес виртуальной машины.  
...
### Предварительно установите клиент `Certbot` от `Electronic Frontier Foundation`.
...
2. Чтобы обновление выполнялось автоматически, необходимо добавить вызов утилиты `certbot` с опцией `renew` в планировщик задач `cron` вашей ВМ.
   ...
   Команда обновления будет запускаться каждый понедельник в 2:30 утра. {% note info %} о результате выполнения будет сохраняться в файл /var/log/le-renewal.log.
...
* В правой части конфигуратора **Environment** (Системное окружение) впишите номер используемой на вашей ВМ версии **Apache** и **OpenSSL**, установите другие требования к набору(**Miscellaneous**): строгая безопасность передачи `HTTP Strict Transport Security` и сшивание OCSP (`OCSP Stapling`). 

{% note info %}

Часть рекомендованных **Mozilla** настроек, которая относится к активации "движка" SSL и указания на место расположения созданного **Let's Encrypt** сертификата и ключа безопасности, вы уже выполнили ранее. 
...
1. Добавьте в **Google Search Console** имя сайта с префиксом `https://`: 
...
2. Добавьте карту сайта в Google Search Console:
...
3. Наблюдайте процесс исключения старых и добавления новых страниц. 
...
3. Добавьте карту `Sitemap` в **Яндекс.Вебмастер**:
...
4. Проверьте, что сайт с протоколом `https` является главным зеркалом:
